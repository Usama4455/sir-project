...
export class CartPage implements OnInit {
  ...
  totalPrice() {
    ...
    this.total = price;
    this.shared.totalCartPrice = price;
    // console.log(price);
  }
  // ============================================================================================
  ...
  async proceedToCheckOut() {
    ...
    this.shared.toast('Please login');
    console.log('Please login');
    } else {
      // <!-- 2.0 updates -->
      console.log('Checkout page test:' + this.config.checkOutPage);
        this.navCtrl.navigateForward('/margin');
    }
  }
  // ============================================================================================
  openProductsPage() {
  ...
  updateCart() {
    ...
    this.shared.cartProducts.forEach((value, index) => {
      ...
      this.config.getWoo('products/' + id + '?' + this.config.productsArguments).then((data: any) => {
        count++;
        const p = data;
        if (p.id == undefined) { this.shared.cartProducts.splice(index, 1); } else if (p.status == 'trash') { this.shared.cartProducts.splice(index, 1); } else {
          value.price = p.price;
          value.name = p.name;
          value.stock_quantity = p.stock_quantity;
          value.tax_status = p.tax_status;
          value.image = p.images[0].shop_thumbnail;
          value.tax_class = p.tax_class;
          value.tax_status = p.tax_status;
          value.on_sale = p.on_sale;
          value.categories = p.categories;

          if (p.stock_quantity == null) { } else if (p.stock_quantity < value.quantity) { value.quantity = p.stock_quantity; } else if (p.stock_status != 'instock') { this.shared.cartProducts.splice(index, 1); }
        ...
        if (count == this.shared.cartProducts.length) {
          this.changingCart();
          this.loading.hide();
          console.log('hide loading from cart');
